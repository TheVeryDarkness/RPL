...
fn bad_async_write_closure<W: AsyncWrite + Unpin + 'static>(
    w: W,
...
async fn async_read_nested_or<R: AsyncRead + Unpin>(
    r: &mut R,
    do_it: bool,
) -> Result<[u8; 1], Error> {
    let mut buf = [0u8; 1];
...
fn returned_value_should_not_lint<T: io::Read + io::Write>(
    s: &mut T,
