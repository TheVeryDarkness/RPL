// Run RPL on a fixed set of crates and collect the warnings.
// This helps observing the impact RPL changes have on a set of real-world code (and not just our
// testsuite).
//
...
/// Represents the actual source code of a crate that we ran "cargo rpl" on
#[derive(Debug)]
struct Crate {
    version: String,
    name: String,
    // path to the extracted sources that RPL can check
    path: PathBuf,
...
impl Crate {
    /// Run `cargo rpl` on the `Crate` and collect and return all the lint warnings that RPL
    /// issued
    #[allow(clippy::too_many_arguments, clippy::too_many_lines)]
    fn run_rpl_lints(
        &self,
        rpl_driver_path: &Path,
        ...
        rpl_args.extend(lint_levels_args.iter().map(String::as_str));
        ...
        cmd.arg(if config.fix { "fix" } else { "check" })
            ...
            .env("RPL_ARGS", rpl_args.join("__RPL_HACKERY__"))
            .env("RPL_DISABLE_DOCS_LINKS", "1");

        if let Some(server) = server {
            // `cargo rpl` is a wrapper around `cargo check` that mainly sets `RUSTC_WORKSPACE_WRAPPER` to
            // `rpl-driver`. We do the same thing here with a couple changes:
            //
            ...
            let status = cmd
                ...
                // Pass the absolute path so `crate::driver` can find `rpl-driver`, as it's executed in various
                // different working directories
                .env("RPL_DRIVER", rpl_driver_path)
        ...
        let all_output = cmd
            ...
            // Roughly equivalent to `cargo rpl`/`cargo rpl --fix`
            .env("RUSTC_WORKSPACE_WRAPPER", rpl_driver_path)
            .output()
            .unwrap();
        ...
        // get all RPL warnings and ICEs
        let mut entries: Vec<RPLCheckOutput> = Message::parse_stream(stdout.as_bytes())
            .filter_map(|msg| match msg {
                Ok(Message::CompilerMessage(message)) => RPLWarning::new(
...
fn build_rpl(release_build: bool) -> String {
    ...
    if !output.status.success() {
        eprintln!("Error: Failed to compile RPL!");
        std::process::exit(1);
    }
...
fn lintcheck(config: LintcheckConfig) {
    ...
    // download and extract the crates, then run RPL on them and collect RPL's warnings
    ...
    let mut rpl_entries: Vec<RPLCheckOutput> = crates
        .par_iter()
        .flat_map(|krate| {
            krate.run_rpl_lints(
                &rpl_driver_path,
    ...
    if let Some(server) = server {
        let server_rpl_entries = server.warnings().map(RPLCheckOutput::RPLWarning);

        rpl_entries.extend(server_rpl_entries);
    }
    ...
    let mut warnings: Vec<RPLWarning> = vec![];
    let mut raw_ices: Vec<RustcIce> = vec![];
    for entry in rpl_entries {
        if let RPLCheckOutput::RPLWarning(x) = entry {
            warnings.push(x);
        } else if let RPLCheckOutput::RustcIce(x) = entry {
            raw_ices.push(x);
        }
    }
    ...
    let text = match config.format {
        OutputFormat::Text | OutputFormat::Markdown => {
            output::summarize_and_print_changes(&warnings, &raw_ices, rpl_ver, &config)
        },
        OutputFormat::Json => {
