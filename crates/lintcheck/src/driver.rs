...
/// 3. Otherwise sends the stderr of running `rpl-driver` to the server
fn run_rpl(addr: &str) -> Option<i32> {
    ...
    // Remove --cap-lints allow so that RPL runs and lints are emitted
    ...
    let output = Command::new(env::var("RPL_DRIVER").expect("missing env RPL_DRIVER"))
        .args(args)
        .stdout(Stdio::inherit())
        .output()
        .expect("failed to run rpl-driver");
...
pub fn drive(addr: &str) {
    process::exit(run_rpl(addr).unwrap_or_else(|| {
        Command::new("rustc")
