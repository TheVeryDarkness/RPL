...
/// A single emitted output from RPL being executed on a crate. It may either be a
/// `RPLWarning`, or a `RustcIce` caused by a panic within RPL. A crate may have many
/// `RPLWarning`s but a maximum of one `RustcIce` (at which point RPL halts execution).
#[derive(Debug)]
pub enum RPLCheckOutput {
    RPLWarning(RPLWarning),
    RustcIce(RustcIce),
}
...
/// A single warning that RPL issued while checking a `Crate`
#[derive(Debug, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct RPLWarning {
    pub name: String,
    pub diag: Diagnostic,
    pub krate: String,
...
impl RPLWarning {
    pub fn new(mut diag: Diagnostic, base_url: &str, krate: &str) -> Option<Self> {
        ...
        if !(name.contains("rpl") || diag.message.contains("rpl"))
...
pub fn summarize_and_print_changes(
    ...
    let mut text = rpl_ver; // RPL version number on top
    text.push_str("\n### Reports\n\n");
...
fn gather_stats(warnings: &[RPLWarning]) -> (String, HashMap<&String, usize>) {
    ...
    // sort by "000{count} {rpl::lintname}"
