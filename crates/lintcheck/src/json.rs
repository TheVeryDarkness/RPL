//! JSON output and comparison functionality for RPL warnings.
//!
//! This module handles serialization of RPL warnings to JSON format,
...
/// Representation of a single RPL warning for JSON serialization.
#[derive(Debug, Deserialize, Serialize)]
struct LintJson {
    /// The lint name e.g. `rpl::bytes_nth`
    name: String,
...
pub(crate) fn output(rpl_warnings: Vec<RPLWarning>) -> String {
    let mut lints: Vec<LintJson> = rpl_warnings
        .into_iter()
        .map(|warning| {
...
fn to_html_id(lint_name: &str) -> String {
    lint_name.replace("rpl::", "").replace('_', "-")
}
