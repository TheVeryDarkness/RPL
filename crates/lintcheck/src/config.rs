...
pub(crate) struct LintcheckConfig {
    ...
    /// File to save the RPL lint results here
    #[clap(skip = "")]
    ...
    /// Runs cargo rpl --fix and checks if all suggestions apply
    #[clap(long, conflicts_with("max_jobs"))]
    pub fix: bool,
    /// Apply a filter to only collect specified lints
    #[clap(long = "filter", value_name = "rpl_lint_name", use_value_delimiter = true)]
    pub lint_filter: Vec<String>,
    /// Check all RPL lints, by default only `rpl::all` and `rpl::pedantic` are checked.
    ...
    /// Run RPL on the dependencies of crates specified in crates-toml
    #[clap(long, conflicts_with("max_jobs"))]
    pub recursive: bool,
...
impl LintcheckConfig {
    pub fn new() -> Self {
        ...
        for lint_name in &mut config.lint_filter {
            *lint_name = format!(
                "rpl::{}",
                lint_name.strip_prefix("rpl::").unwrap_or(lint_name).replace('_', "-")
            );
        }
