...
fn process_stream(
    ...
    let messages = stderr
        ...
        .filter_map(|diag| RPLWarning::new(diag, &base_url, &driver_info.package_name));
...
pub(crate) struct LintcheckServer {
    ...
    receiver: Receiver<RPLWarning>,
    sender: Arc<Sender<RPLWarning>>,
}
...
impl LintcheckServer {
    pub fn spawn(options: RecursiveOptions) -> Self {
        ...
        let (sender, receiver) = crossbeam_channel::unbounded::<RPLWarning>();
    ...
    pub fn warnings(self) -> impl Iterator<Item = RPLWarning> {
