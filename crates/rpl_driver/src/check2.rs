...
pub(crate) fn walk2<'pcx, 'tcx>(tcx: TyCtxt<'tcx>, pcx: PatCtxt<'pcx>) {
    ...
    // for matched in matched {                         let bottom = matched.bottom();
    //                         // let matched = NormalizedMatched::new(&matched, label_map,
...
impl<'tcx, 'pcx> MatchComposedPattern<'pcx, 'tcx> for CheckFnsCtxt<'pcx, 'tcx> {
